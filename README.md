...
┌──────────────┐   spawn (1 per script)   ┌──────────────────┐
│   pyman      │ ───────────────────────► │  pyman-worker    │
│  (egui GUI)  │  capture stdout/stderr   │  (pyo3 + CPython)│
...
| crate | 作用 |
|-------|------|
| `pyman` | GUI 进程（既是 lib 也是 bin）。`app` 模块渲染界面并管理条目列表；`supervisor` 启动子进程、按行读取它们的 stdout/stderr、轮询退出状态；`history` 负责把脚本列表持久化到磁盘并在启动时加载。**自身不嵌入 Python。** |
| `pyman-worker` | 叶子进程。用 `pyo3`（`auto-initialize` 特性）嵌入 CPython，把脚本作为 `__main__` 执行，正确设置 `sys.argv`。每个脚本对应一个 worker 进程。 |
...
GUI 进程通过查找“自身可执行文件的同级目录”来定位 worker 二进制（回退到 PATH 中的 `pyman-worker`），因此发布时把两个 exe 放在同一目录即可。
...
- `target/release/pyman`（GUI）
- `target/release/pyman-worker`（脚本执行进程）
...
│  ├─ pyman/                  # GUI 进程
│  │  ├─ Cargo.toml
│  │  └─ src/{main,app,supervisor}.rs
│  └─ pyman-worker/           # 脚本执行进程 (pyo3)
│     ├─ Cargo.toml
│     └─ src/main.rs
